...
type ShowContext = {
  ...
  settlement_total_amount?: string | number | null;
...
export default function EconomicsTab({
  ...
  <div className="mt-1 flex items-baseline justify-between gap-5">
    <h2 className="text-xl font-black text-[#191917]">
      Was kam rein?
    </h2>

    <div className="shrink-0 text-right text-2xl font-black text-[#191917]">
      {formatEuro(revenue)}
    </div>
  </div>
</div>

            {(showRevenue.automatic || revenueItems.length > 0) && (
              <div className="mt-5 space-y-2">
                ...
                {showRevenue.automatic && (
                  <div className="grid grid-cols-[165px_1fr_110px_34px] gap-2">
                    ...
                    <div className="flex h-11 min-w-0 items-center rounded-xl border border-[#e2ddd1] bg-[#faf8f2] px-3 text-sm font-semibold text-[#25231f]">
                      <span className="truncate">
                        {showRevenue.detail || showRevenue.label}
                      </span>
                    </div>
                ...
                {revenueItems.map((item, index) => (
                  <div key={index} className="grid grid-cols-[165px_1fr_110px_34px] gap-2">
                    <div className="flex h-11 items-center rounded-xl border border-[#ddd7ca] bg-[#faf8f2] px-3 text-sm font-semibold text-[#25231f]">
      ...
      <div className="flex flex-col gap-3 rounded-[22px] border border-[#e2ddd1] bg-white px-5 py-4 shadow-[0_8px_20px_rgba(45,40,28,.04)] sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="font-black text-[#20201d]">
            {completedAt
              ? "✓ Wirtschaftlichkeit abgeschlossen"
              : saved
                ? "✓ Gespeichert"
                : "Wirtschaftlichkeit"}
          </div>
          <div className={`mt-1 text-xs font-semibold ${saveError ? "text-[#9d4949]" : completedAt || saved ? "text-[#5d7100]" : "text-[#88857d]"}`}>
            {saveError
              ? saveError
              : completedAt
                ...
                : saving
                  ...
                  : saved
                    ...
                    : "Änderungen speichern und anschließend abschließen."}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {completedAt ? (
            <form action={reopenAction}>
              ...
              <button
                type="submit"
                className="rounded-full border border-[#ddd7ca] bg-white px-6 py-3 text-sm font-black text-[#5f5b54] transition hover:border-[#c9d65c] hover:bg-[#faf8f2]"
              >
                Wieder öffnen
              </button>
            </form>
            ...
            <form action={completeAction}>
              ...
              <button
                type="submit"
                className="rounded-full bg-[#dbe76e] px-6 py-3 text-sm font-black text-[#20201d] transition hover:bg-[#cdd95f]"
              >
...
function getShowRevenue(show?: ShowContext) {
  const settlement = toNumber(show?.settlement_total_amount ?? undefined);
  if (show?.settlement_confirmed && settlement >= 0) {
    return {
      amount: settlement,
      label: "Künstlerumsatz / Abrechnung",
      detail: "aus Nachbereitung · automatisch übernommen",
      automatic: true,
    };
  }
